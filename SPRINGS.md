...
> <!--path:相对上下文路径的相对路径，view-name：就是要访问的视图名称-->
...
> ```
>
> 需要注意的是，`<view-controller>`标签的`path`值指的是相对**上下文路径**而言，上下文路径就是“/”，与你在tomcat中设置的上下文路径不是一个值。
>
> 如下：（图中的 ）
>
> ![image-20220211160409551](https://masuo-github-image.oss-cn-beijing.aliyuncs.com/image/20220211160409.png)
...
> 在HTTP协议里，有四个表示操作的动词：GET、POST、PUT、DELETE。
>
> 它们分别对应四种基本操作：
>
> | GET      | POST     | PUT      | DELETE   |
...
> | 获取资源 | 新建资源 | 更新资源 | 删除资源 |
>
> REST风格提倡URL地址使用统一的风格设计，从前到后每个资源用“/”分开，尽量不要使用问号键值对得方式携带请求参数，而是将要发送给服务器的数据作为URL地址的一部分，以保证整体风格的一致性。
...
| 新增 | addUser             | /user，POST请求     |
| 更新 | updateUser          | /user，PUT请求      |
| 删除 | deleteUserById?id=1 | /user/1，DELETE请求 |

##### HiddenHttpMethodFilter

> 但是由于浏览器只支持`POST`和`GET`请求，如何发送`PUT`和`DELETE`请求就成了我们唯一的阻碍。
>
> 正好，SpringMVC提供了`HiddenHttpMethodFilter`帮助我们实现发送`PUT`和`DELETE`请求。
>
> 但是他有两个前提
>
> - 原请求必须是POST请求
> - 必须携带参数`_method`，值为`PUT`或`DELETE`.
...
> 注意，在有多个过滤器的时候，我们要保证字符过滤器设置在第一个，不然就会失效。
